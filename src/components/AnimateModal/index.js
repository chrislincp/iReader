...
export default class AnimateModal extends Component {
  static defaultProps = {
    visible: false,
    animationType: 'alert',
  ...
  static propTypes = {
    ...
    // 动画类型：none(没有)| fade（渐隐渐显）| slide（从底部出现）|
    // slide-down（从顶部出现）| scale(放缩) | fade_scale(渐隐渐现放缩)|alert(弹窗)
          ...
          if (visible) {
            ...
            this.animDialog = Animated.parallel([
              ...
              Animated.timing(scale, {
                ...
                duration: animationDuration,
                easing:
                  visible && springEffect ? Easing.elastic(0.8) : undefined,
              }),
            ]);
          } else {
            this.animDialog = Animated.timing(opacity, {
  ...
  render() {
    ...
    const {
      children, animationType, style, maskStyle,
    } = this.props;
    ...
    return (
      <Modal visible transparent onRequestClose={this._maskClose}>
        <View style={[styles.container, style]}>
          ...
          <Animated.View
            style={[styles.content, animationStyleMap[animationType]]}
          >
            {children}
          </Animated.View>
        </View>
